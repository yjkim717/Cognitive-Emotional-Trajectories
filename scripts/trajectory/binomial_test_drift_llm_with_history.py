#!/usr/bin/env python3
"""
Binomial Test: Human vs LLM_with_history Comparison on Embedding Drift
=======================================================================

Compare Human vs LLM_with_history drift using total drift (sum of all year-to-year drifts per author).
...
H0: P(Human drift > LLM_with_history drift) = 0.5
H1: P(Human drift > LLM_with_history drift) > 0.5

Uses paired comparisons: each human author matched with LLM_with_history author (same author_id, field, domain).
No FDR correction applied - uses raw p-values for significance testing (aligned with Combined_Results.md).
"""
...
def compare_human_vs_llm_drift(
    ...
    """
    Compare Human vs LLM_with_history total drift for paired authors.
    ...
    Args:
        ...
        llm_df: DataFrame with LLM_with_history drift (with year_from, year_to, drift columns)
        rep_space: Representation space name (ce, tfidf, sbert)
        model: Model name (DS, CL35, G4OM)
    ...
    for (author_id, field, domain), human_group in human_df.groupby(merge_cols):
        ...
        # Binary outcome: Human > LLM_with_history
        ...
        results.append({
            ...
            "llm_with_history_total_drift": llm_common,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Binomial test for Human vs LLM_with_history drift comparison"
    )
    parser.add_argument(
        "--domains",
        nargs="+",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "results" / "drift" / "binomial_test_llm_with_history",
    ...
    print("=" * 80)
    print("Binomial Test: Human vs LLM_with_history Embedding Drift")
    print("=" * 80)
    ...
    print(f"H0: P(Human drift > LLM_with_history drift) = 0.5")
    print(f"H1: P(Human drift > LLM_with_history drift) > 0.5")
    ...
    for domain in args.domains:
        ...
        # Load LLM_with_history data and compare
        for provider in args.providers:
            for level in args.levels:
                ...
                llm_drifts = {}
                for rep_space in REP_SPACES:
                    llm_df = load_drift_data(domain, rep_space, "llm_with_history", provider, level)
                    if not llm_df.empty:
                        llm_drifts[rep_space] = llm_df
                        print(f"    {rep_space}: {len(llm_df)} drift measurements")
                    else:
                        print(f"    ⚠️  {rep_space}: No LLM_with_history data found")
    ...
    test_results_df["sort_order"] = test_results_df["rep_space"].map(rep_space_order)
    test_results_df = test_results_df.sort_values(["sort_order", "model", "level"]).drop("sort_order", axis=1)
    ...
    # Summary statistics (raw p-values)
    print("\n" + "=" * 80)
    print("SUMMARY")
    print("=" * 80)

    for rep_space in REP_SPACES:
        space_results = test_results_df[test_results_df["rep_space"] == rep_space]
        if space_results.empty:
            continue

        n_significant = space_results["significant"].sum()
        ...
        print(f"  Mean p-value: {space_results['pvalue'].mean():.4f}")
    ...
    comparisons_output = args.output_dir / "llm_with_history_drift_comparisons_detailed.csv"
    comparisons_df.to_csv(comparisons_output, index=False)
    print(f"\n✅ Saved detailed comparisons to: {comparisons_output}")

    # Save test results (raw p-values, no FDR)
    results_output = args.output_dir / "binomial_test_llm_with_history_drift_results.csv"
