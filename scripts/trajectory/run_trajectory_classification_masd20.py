#!/usr/bin/env python3
"""
Human vs. LLM (LV3 only) classification on 20 CE MASD_norm features.

Configuration:
- Features: ONLY 20 MASD_norm features (one per CE/text feature)
- Model: RandomForestClassifier
- CV: 5-fold GroupKFold (group by author_id)

Outputs:
- CSV: plots/trajectory/combined/classification_masd20_<domains>_<level>.csv
- Feature importance CSV: plots/trajectory/combined/classification_masd20_importance_<domains>_<level>.csv
- Feature importance barplot: plots/trajectory/combined/classification_masd20_importance_<domains>_<level>.png
- Metrics barplot (Accuracy / ROC-AUC / F1):
  plots/trajectory/combined/classification_masd20_metrics_<domains>_<level>.png
"""
...
def select_masd20_features(df: pd.DataFrame) -> pd.DataFrame:
    """Select the 20 MASD_norm features, ensuring they exist in the DataFrame."""
    available = [col for col in MASD20_FEATURES if col in df.columns]
    missing = [col for col in MASD20_FEATURES if col not in df.columns]
    if missing:
        print(f"⚠ Warning: missing MASD_norm features (will be skipped): {missing}")
    if not available:
        raise ValueError("No MASD20 features found in DataFrame.")
    return df[available].fillna(0.0)


def evaluate_masd20(df: pd.DataFrame, X_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Run RandomForest + GroupKFold on 20 MASD_norm features.

    Returns:
        - results_df: one row with metrics for masd20
...
def plot_importance_bar(importance_df: pd.DataFrame, out_path: Path) -> None:
    """Plot bar chart of feature importance for the 20 MASD_norm features (percentage)."""
    ...
    plt.title("RandomForest feature importance (20 MASD_norm features)", fontsize=13, fontweight="bold", pad=10)
...
def plot_metrics_bar(results_df: pd.DataFrame, out_path: Path) -> None:
    """Plot bar chart of accuracy / ROC-AUC / F1 for the masd20 model."""
    ...
    plt.title("Human vs LLM (20 MASD_norm features)", fontsize=13, fontweight="bold", pad=10)
    plt.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Human vs LLM (LV3) classification on 20 MASD_norm features.")
    parser.add_argument(
        "--domains",
        nargs="+",
        choices=DOMAINS,
...
def main() -> None:
    ...
    for level in args.level:
        ...
        print(f"\n{'=' * 80}")
        print(f"=== Classifying Human vs LLM ({level}) on 20 MASD_norm features ===")
        print(f"{'=' * 80}")
        print(f"Domains: {', '.join(args.domains).upper()}")
        print(f"Models:  {', '.join(args.models)}")

        # Select 20 MASD_norm features
        X_df = select_masd20_features(df)
        print(f"Using {X_df.shape[1]} MASD_norm features: {list(X_df.columns)}")

        # Evaluate + importance
        results_df, importance_df = evaluate_masd20(df, X_df)
        ...
        # Save metrics
        out_path = out_dir / f"classification_masd20_{suffix}.csv"
        results_df.to_csv(out_path, index=False)
        print(f"\nResults → {out_path}")

        # Save metrics bar plot (Accuracy / ROC-AUC / F1)
        metrics_png_path = out_dir / f"classification_masd20_metrics_{suffix}.png"
        plot_metrics_bar(results_df, metrics_png_path)

        # Save importance CSV
        imp_csv_path = out_dir / f"classification_masd20_importance_{suffix}.csv"
        importance_df.to_csv(imp_csv_path, index=False)
        print(f"Feature importance CSV → {imp_csv_path}")

        # Save importance bar plot
        imp_png_path = out_dir / f"classification_masd20_importance_{suffix}.png"
    ...
    if len(all_results) > 1:
        ...
        combined_results_path = out_dir / f"classification_masd20_{suffix}.csv"
        combined_results.to_csv(combined_results_path, index=False)
        print(f"\nCombined results → {combined_results_path}")
        
        combined_importance_path = out_dir / f"classification_masd20_importance_{suffix}.csv"
