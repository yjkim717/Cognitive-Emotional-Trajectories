#!/usr/bin/env python3
"""
Aggregate SBERT vectors into per-author per-year representations.

For each domain (academic/blogs/news) and each split (human, per-provider LV),
this script reads the generated `sbert_vectors.csv`, parses years
from filenames, averages multiple samples within the same year, and writes
`sbert_author_yearly_vectors.csv` beside the source file.
"""
...
LEVELS = ("LV1", "LV2", "LV3")
INPUT_FILENAME = "sbert_vectors.csv"
OUTPUT_FILENAME = "sbert_author_yearly_vectors.csv"
...
def aggregate_vectors(df: pd.DataFrame) -> pd.DataFrame:
    """Average SBERT vectors per (author, year)."""
    vector_cols = [col for col in df.columns if col.startswith("sbert_")]
    if not vector_cols:
        raise ValueError("No SBERT vector columns found (sbert_*).")
...
def process_split(csv_path: Path) -> None:
    ...
    aggregated = aggregate_vectors(df)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate SBERT vectors into per-author per-year averages."
    )
    parser.add_argument(
        "--domains",
        nargs="+",
...
def main() -> None:
    ...
    for domain in args.domains:
        print(f"\n=== Aggregating SBERT yearly vectors for {domain} ===")
