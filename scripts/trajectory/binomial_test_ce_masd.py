#!/usr/bin/env python3
"""
Binomial Test: Human vs LLM Comparison on 20 CE MASD_norm Features
=============================================================

Compare Human vs LLM MASD_norm (Normalized Mean Absolute Successive Differences) for 20 CE features.
Input source: trajectory_features_combined.csv

Tests:
- 20 CE features (each feature separately)

H0: P(Human MASD > LLM MASD) = 0.5
H1: P(Human MASD > LLM MASD) > 0.5
...
def compare_human_vs_llm_masd(
    ...
    """
    Compare Human vs LLM MASD for a specific feature using paired authors.
    
    Args:
        human_df: DataFrame with human data (with feature column)
        llm_df: DataFrame with LLM data (with feature column)
        feature: Feature name (e.g., "Agreeableness_masd")
    ...
    # Binary outcome: Human MASD_norm > LLM MASD_norm
    ...
    results_df = pd.DataFrame({
        ...
        "feature": feature,
        "model": model,
        "human_masd_norm": merged_clean[f"{feature}_human"],
        "llm_masd_norm": merged_clean[f"{feature}_llm"],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Binomial test for Human vs LLM CE MASD_norm comparison (LV3 only)"
    )
    parser.add_argument(
        "--domains",
        nargs="+",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "results" / "ce_masd_norm" / "binomial_test",
    ...
    print("=" * 80)
    print("Binomial Test: Human vs LLM CE MASD_norm Comparison (LV3)")
    print("=" * 80)
    print(f"Domains: {args.domains}")
    print(f"LLM Providers: {args.providers}")
    print(f"LLM Level: {LEVEL} (fixed)")
    print(f"Features: {len(MASD20_FEATURES)} CE MASD_norm features")
    print(f"Input: trajectory_features_combined.csv")
    print(f"H0: P(Human MASD_norm > LLM MASD_norm) = 0.5")
    print(f"H1: P(Human MASD_norm > LLM MASD_norm) > 0.5")
    ...
    for domain in args.domains:
        ...
        # Check which MASD_norm features are available
        available_features = [f for f in MASD20_FEATURES if f in human_df.columns]
        missing_features = [f for f in MASD20_FEATURES if f not in human_df.columns]
        if missing_features:
            print(f"  ⚠️  Missing features: {missing_features}")
        print(f"  Human: {len(human_df)} authors, {len(available_features)}/{len(MASD20_FEATURES)} features available")
        ...
        for provider in args.providers:
            ...
            for feature in available_features:
                ...
                comparisons = compare_human_vs_llm_masd(
                    human_df,
                    llm_df,
                    feature,
                    provider,
    ...
    for feature in MASD20_FEATURES:
        for provider in args.providers:
    ...
    feature_order = {feat: i for i, feat in enumerate(MASD20_FEATURES)}
    ...
    # Print MASD_norm comparison statistics for all 20 features
    print("\n" + "=" * 80)
    print("MASD_norm COMPARISON STATISTICS (All 20 CE Features)")
    print("=" * 80)
    ...
    for provider in args.providers:
        ...
        print(f"\n{provider}/{LEVEL}:")
        print(f"{'Feature':<40s} {'N':>5s} {'Human MASD_norm Mean':>22s} {'LLM MASD_norm Mean':>22s} {'Human MASD_norm Std':>22s} {'LLM MASD_norm Std':>22s} {'Win Rate':>10s}")
        print("-" * 140)
        ...
        for feature in MASD20_FEATURES:
            ...
            human_masd_norm_mean = feature_comparisons["human_masd_norm"].mean()
            llm_masd_norm_mean = feature_comparisons["llm_masd_norm"].mean()
            human_masd_norm_std = feature_comparisons["human_masd_norm"].std()
            llm_masd_norm_std = feature_comparisons["llm_masd_norm"].std()
            win_rate = feature_comparisons["human_wins"].mean()
            n = len(feature_comparisons)
            
            print(f"{feature:<40s} {n:>5d} {human_masd_norm_mean:>22.6f} {llm_masd_norm_mean:>22.6f} {human_masd_norm_std:>22.6f} {llm_masd_norm_std:>22.6f} {win_rate:>10.3f}")
    ...
    comparisons_output = args.output_dir / "ce_masd_norm_comparisons_detailed.csv"
    comparisons_df.to_csv(comparisons_output, index=False)
    print(f"\n✅ Saved detailed comparisons to: {comparisons_output}")
    
    # Save test results
    results_output = args.output_dir / "binomial_test_ce_masd_norm_results.csv"
